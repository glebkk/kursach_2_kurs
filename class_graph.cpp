...
string bfs_code = R"(
------------------------------------------------------------------------------------------------
#include <queue> // для очереди, queue 
adj_matrix - матрица смежности
count_vertex - кол-во вершин
------------------------------------------------------------------------------------------------
...
string kruskala_code = R"(
------------------------------------------------------------------------------------------------
#include <algorithm> // для сортировки, sort()

struct node{ // структура ребра
	int x;
	int y;
	int weight;

	bool operator<( const node& node) { // для сортировки
		return weight < node.weight;
	}
};

node_list - список ребёр
count_vertex - кол-во вершин
vector<node> node_list - массив рёбер
------------------------------------------------------------------------------------------------
...
int min = 999, x = 0, y = 0;
int current_vertex = 0; // текушая вершина
int count_nodes_ost = 0; // кол-во рёбер остовного дерева(должно быть меньше на 1 кол-ва вершин)
int weight_ost = 0; // вес отсовного дерева

vector<bool> visited(count_vertex, false); // массив посещенных вершин
visited[current_vertex] = true; // первую вершину помечаем посещенной
...
string dijkstra_code = R"(
------------------------------------------------------------------------------------------------
adj_matrix - матрица смежности
count_vertex - кол-во вершин
current_vertex - текушая вершина
------------------------------------------------------------------------------------------------
...
private:
	...
	void show_code(string algorithm_code) {
		
		bool exit = false;

		while (!exit) {
			cout << "\rВывести код: [y/n]";
			char code = _getch();
			switch (code) {
			case (int('y')):
			case (int('Y')):
